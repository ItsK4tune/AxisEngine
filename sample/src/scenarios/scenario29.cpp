...
void SampleState::LoadScene29()
{
    ...
    auto uiLogo = LoadUiTexture(res, "ui_logo_29", "include/engine/asset/project/logo.png");

    m_S29RootPanel = EntityBuilder(scene, res, "scenario")
                          .WithName("ResponsiveRootPanel")
                          .WithUITransform(glm::vec2(0.0f), glm::vec2(0.0f), 8)
                          .WithUIAnchors(glm::vec2(0.58f, 0.08f), glm::vec2(0.98f, 0.92f))
                          .WithUIOffsets(glm::vec2(0.0f), glm::vec2(0.0f))
    ...
    auto leftColumn = EntityBuilder(scene, res, "scenario")
                          ...
                          .WithParent(m_S29RootPanel)
                          .WithUITransform(glm::vec2(0.0f), glm::vec2(280.0f, 260.0f), 9)
    ...
    EntityBuilder(scene, res, "scenario")
        ...
        .WithParent(leftColumn)
        .WithUITransform(glm::vec2(0.0f), glm::vec2(260.0f, 48.0f), 10)
    ...
    EntityBuilder(scene, res, "scenario")
        ...
        .WithParent(leftColumn)
        .WithUITransform(glm::vec2(0.0f), glm::vec2(260.0f, 150.0f), 10)
        .WithUIText(
    ...
    auto metricsRow = EntityBuilder(scene, res, "scenario")
                          ...
                          .WithParent(leftColumn)
                          .WithUITransform(glm::vec2(0.0f), glm::vec2(260.0f, 60.0f), 10)
    ...
    for (int i = 0; i < 3; ++i)
    {
        EntityBuilder(scene, res, "scenario")
            ...
            .WithParent(metricsRow)
            .WithUITransform(glm::vec2(0.0f), glm::vec2(80.0f, 52.0f), 11)
    ...
    auto rightColumn = EntityBuilder(scene, res, "scenario")
                           ...
                           .WithParent(m_S29RootPanel)
                           .WithUITransform(glm::vec2(0.0f), glm::vec2(240.0f, 260.0f), 9)
    ...
    auto preview = EntityBuilder(scene, res, "scenario")
                       ...
                       .WithParent(rightColumn)
                       .WithUITransform(glm::vec2(0.0f), glm::vec2(220.0f, 170.0f), 10)
                       .WithUIRenderer("ui_preview_29", glm::vec4(0.18f, 0.21f, 0.24f, 0.95f))
                       .Build();
    if (auto* renderer = scene.registry.try_get<UIRendererComponent>(preview))
        renderer->texture = uiLogo;

    EntityBuilder(scene, res, "scenario")
        .WithName("ResponsivePreviewCaption")
        .WithParent(rightColumn)
        .WithUITransform(glm::vec2(0.0f), glm::vec2(220.0f, 64.0f), 11)
