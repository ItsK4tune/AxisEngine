...
void SampleState::OnUpdate(float dt)
{
    ...
    {
        if (m_S32PortalRotationActive)
        {
            m_S32PortalAngle += dt * 0.5f;
            if (m_S32PortalTarget != entt::null)
            {
...
void SampleState::DrawGUI()
{
    ...
    else if (m_CurrentScenario == 32)
    {
        ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.5f, 1.0f), "Scenario 32: Outline & Portal Controls");
        ImGui::ColorEdit3("Outline Color", m_S32OutlineColor);
