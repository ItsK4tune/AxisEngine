...
void GameState::OnUpdate(float dt)
{
    ...
    if (!io)
        return;  // headless: no input processing
    ...
    if (input.GetActionDown("Pause"))
        this->Get<RuntimeCore>().GetStateMachine().PushState(std::make_unique<PauseState>());
    ...
    if (input.GetActionDown("Select"))
    {
        ...
        if (camEntity != entt::null)
        {
            ...
            glm::vec3 camPos = (EntityManager::TryGetComponent<PositionComponent>(this->GetScene(), camEntity))
                                   ? EntityManager::GetComponent<PositionComponent>(this->GetScene(), camEntity).value
                                   : glm::vec3(0.0f);
            ...
            if (hit.hasHit)
            {
                ...
                else
                {
                    ...
                    {
                        if (auto* follower = EntityManager::TryGetComponent<PathFollowerComponent>(this->GetScene(),
                                                                                                   m_SelectedEntity))
                        {
                            follower->lockXPitch = true;
                            follower->lockZRoll = true;
                            this->GetSystem<NavigationSystem>().MoveTo(this->GetScene(), m_SelectedEntity,
                                                                       hit.hitPoint);
                            LOGGER_INFO("GameState") << "Move Command: target=(" << hit.hitPoint.x << ","
                    ...
                    if (decalSys)
                    {
                        ...
                        if (crackTex == 0)
                            crackTex = decalSys->LoadDecalTexture("resources/textures/decal_crack.png");
                        ...
                        if (glm::abs(glm::dot(hit.hitNormal, up)) > 0.99f)
                            up = glm::vec3(1, 0, 0);
                        ...
                        LOGGER_INFO("GameState")
                            << "Spawned Decal at " << hit.hitPoint.x << "," << hit.hitPoint.y << "," << hit.hitPoint.z;
                        ...
                        // auto& pComp = EntityManager::AddComponent<ParticleEmitterComponent>(this->GetScene(),
                        // particleEnt);

                        // pComp.isActive = true; pComp.lifetime = 0.5f;
                        // pComp.emitter.Initialize(300);
                        // pComp.emitter.Texture =
                        // this->Get<ResourceManager>().GetTextureAuto("resources/textures/particle_star.png");
                        // pComp.emitter.SpawnRate = 400.0f; pComp.emitter.LifeTime = 0.8f;
                        // pComp.emitter.StartSize = 0.5f; pComp.emitter.EndSize = 0.0f;
                        // pComp.emitter.MinVelocity = glm::vec3(-3, 1, -3); pComp.emitter.MaxVelocity = glm::vec3(3, 6,
                        // 3); pComp.emitter.StartColor = glm::vec4(1.0f, 0.8f, 0.4f, 1.0f); pComp.emitter.EndColor =
                        // glm::vec4(1.0f, 0.2f, 0.0f, 0.0f); pComp.emitter.Shape =
        ...
        if (camEntity != entt::null)
        {
            ...
            if (hit.hasHit)
            {
                if (wheelMove > 0)
                {
                    auto* audioSvc = this->Resolve<AudioService>();
                    if (audioSvc)
                        audioSvc->Play3D("resources/audios/3dsound.mp3", hit.hitPoint, false);
                }
                else
                {
                    auto* info = EntityManager::TryGetComponent<InfoComponent>(this->GetScene(), hit.entity);
                    if (info && (info->name == "WallTest" || info->tag == "terrain"))
                    {
                        ...
                        pPos.value = hit.hitPoint;
                        auto& pComp =
                            EntityManager::AddComponent<ParticleEmitterComponent>(this->GetScene(), particleEnt);
    ...
    for (auto entity : demoView)
    {
        ...
        if (info.tag == "enemy")
            mat.desc.ports.data[0] = 1.0f;
        else if (info.tag == "ally")
            mat.desc.ports.data[0] = 2.0f;
        else
            mat.desc.ports.data[0] = 0.0f;
