...
inline void RequireEqual(const auto& actual, const auto& expected, const char* exprA, const char* exprB,
                         const char* file, int line)
{
    if (actual != expected)
    {
        std::ostringstream ss;
        ss << "REQUIRE_EQUAL failed: " << exprA << " == " << exprB << " (actual: " << actual
           << ", expected: " << expected << ")"
           << " (" << file << ":" << line << ")";
        throw AssertionError(ss.str());
    }
...
#define REQUIRE(expr) axis_test::Require((expr), #expr, __FILE__, __LINE__)

#define REQUIRE_EQUAL(a, b) axis_test::RequireEqual((a), (b), #a, #b, __FILE__, __LINE__)
...
#define REQUIRE_NULL(ptr) axis_test::Require((ptr) == nullptr, #ptr " should be null", __FILE__, __LINE__)

#define REQUIRE_NOT_NULL(ptr) axis_test::Require((ptr) != nullptr, #ptr " should not be null", __FILE__, __LINE__)
