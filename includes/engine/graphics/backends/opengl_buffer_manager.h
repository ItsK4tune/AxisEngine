...
public:
    ...
    void BindBuffer(Graphics::BufferType target, unsigned int buffer) override { glBindBuffer(GLTranslator::ToGL(target), buffer); }

    void BufferData(Graphics::BufferType target, size_t size, const void *data, Graphics::BufferUsage usage) override
    {
        glBufferData(GLTranslator::ToGL(target), static_cast<GLsizeiptr>(size), data, GLTranslator::ToGL(usage));
    }

    void BufferSubData(Graphics::BufferType target, size_t offset, size_t size, const void *data) override
    {
    ...
    void BindBufferBase(Graphics::BufferType target, unsigned int index, unsigned int buffer) override
    {
        glBindBufferBase(GLTranslator::ToGL(target), index, buffer);
    }

    void BindBufferRange(Graphics::BufferType target, unsigned int index, unsigned int buffer, size_t offset, size_t size) override
    {
    ...
    void VertexAttribPointer(unsigned int index, int size, Graphics::DataType type, bool normalized, int stride, const void *pointer) override
    {
        glVertexAttribPointer(index, size, GLTranslator::ToGL(type), normalized ? GL_TRUE : GL_FALSE, stride, pointer);
    }

    void VertexAttribIPointer(unsigned int index, int size, Graphics::DataType type, int stride, const void *pointer) override
    {
