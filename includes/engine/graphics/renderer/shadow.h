...
#include <memory>
#include <vector>
#include <graphics/core/gpu_resources.h>
...
private:
    ...
    std::unique_ptr<Graphics::GPUFramebuffer> m_ShadowFBO_Dir[MAX_DIR_LIGHTS_SHADOW];
    std::unique_ptr<Graphics::GPUTexture> m_ShadowMap_Dir[MAX_DIR_LIGHTS_SHADOW];

    std::unique_ptr<Graphics::GPUFramebuffer> m_ShadowFBO_Point[MAX_POINT_LIGHTS_SHADOW];
    std::unique_ptr<Graphics::GPUTexture> m_ShadowMap_Point[MAX_POINT_LIGHTS_SHADOW];

    std::unique_ptr<Graphics::GPUFramebuffer> m_ShadowFBO_Spot[MAX_SPOT_LIGHTS_SHADOW];
    std::unique_ptr<Graphics::GPUTexture> m_ShadowMap_Spot[MAX_SPOT_LIGHTS_SHADOW];
