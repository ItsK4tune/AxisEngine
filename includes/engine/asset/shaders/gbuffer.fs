...
{    
    ...
    // Mix in custom port 0 as a team-color override if used (legacy compatibility or engine feature)
    // In our case, team color is handled by u_CustomPorts[0]
    ...
    // Add selection glow from port 1
    if (u_CustomPorts[1] > 0.5) {
        result += vec3(0.3, 0.3, 0.1); // Subtle yellow glow
    }
