...
void btITaskScheduler::activate()
{
	...
	if (!m_isActive)
	{
		gThreadCounter.mCounter = m_savedThreadCounter;  
		m_isActive = true;
	}
}
...
void btITaskScheduler::deactivate()
{
	if (m_isActive)
	{
		m_savedThreadCounter = gThreadCounter.mCounter;  
		m_isActive = false;
	}
}
...
void btParallelFor(int iBegin, int iEnd, int grainSize, const btIParallelForBody& body)
{
	...
	btAssert(gBtTaskScheduler != NULL);  
...
{
	int m_numThreads;
	concurrency::combinable<btScalar> m_sum;  
public:
