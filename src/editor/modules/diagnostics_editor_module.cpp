...
void DiagnosticsEditorModule::Shutdown()
{
...
void DiagnosticsEditorModule::Render(Scene& scene)
{
    if (!m_Enabled)
        return;
    ...
    // Legacy passive transform indicators are retained temporarily for source
    // compatibility; the interactive TransformGizmo owns transform rendering.
    const entt::entity selectedEntity = ServiceLocator::Instance().Require<EditorSelection>().GetPrimary();
    if (false && selectedEntity != entt::null && scene.IsValid(selectedEntity))
    {
        glm::vec3 pos(0.0f);
        if (auto* tr = scene.TryGetComponent<WorldTransformComponent>(selectedEntity))
            pos = glm::vec3(tr->worldMatrix[3]);
        else if (auto* p = scene.TryGetComponent<PositionComponent>(selectedEntity))
            pos = p->value;
        ...
        if (ioHandler)
        {
            ...
            bool alt = keyboard.GetRawKey(Key::LeftAlt) || keyboard.GetRawKey(Key::RightAlt);
            bool ctrl = keyboard.GetRawKey(Key::LeftControl) || keyboard.GetRawKey(Key::RightControl);
            bool shift = keyboard.GetRawKey(Key::LeftShift) || keyboard.GetRawKey(Key::RightShift);

            float indicatorScale = 1.0f;
            if (auto* s = scene.TryGetComponent<ScaleComponent>(selectedEntity))
            {
                indicatorScale =
...
void DiagnosticsEditorModule::ProcessInput(KeyboardManager& keyboard)
{
    if (!m_Enabled)
        return;
    ...
    if (IsEditorShortcutPressed(keyboard, Key::F8, EditorModifier::None, m_F8Pressed, inputBlocked))
    {
        auto cm = sl.Resolve<ConfigManager>();
        if (cm)
        {
    ...
    if (IsEditorShortcutPressed(keyboard, Key::F9, EditorModifier::None, m_F9Pressed, inputBlocked))
    {
        auto cm = sl.Resolve<ConfigManager>();
        if (cm)
        {
