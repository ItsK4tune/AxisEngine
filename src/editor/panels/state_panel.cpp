...
void StatePanel::OnImGui(Scene& scene)
{
    ...
    if (currentState)
    {
        ...
        if (rawName.find("MenuState") != std::string::npos) stateName = "MenuState";
        else if (rawName.find("AimGameState") != std::string::npos) stateName = "AimGameState";
        else if (rawName.find("SettingsState") != std::string::npos) stateName = "SettingsState";
        else if (rawName.find("PauseState") != std::string::npos) stateName = "PauseState";
        else if (rawName.find("ResultState") != std::string::npos) stateName = "ResultState";
        else stateName = rawName;
    }
    ...
    if (ImGui::IsWindowHovered() && ImGui::GetIO().MouseWheel != 0.0f)
    {
        ...
        ImVec2 mouse_in_canvas = ImVec2(mouse_pos.x - canvas_pos.x - scrolling.x, mouse_pos.y - canvas_pos.y - scrolling.y);
    ...
    for (float x = std::fmod(scrolling.x, grid_size); x < canvas_size.x; x += grid_size)
        draw_list->AddLine(ImVec2(canvas_pos.x + x, canvas_pos.y), ImVec2(canvas_pos.x + x, canvas_end.y), IM_COL32(35, 35, 45, 255));
    for (float y = std::fmod(scrolling.y, grid_size); y < canvas_size.y; y += grid_size)
        draw_list->AddLine(ImVec2(canvas_pos.x, canvas_pos.y + y), ImVec2(canvas_end.x, canvas_pos.y + y), IM_COL32(35, 35, 45, 255));
    ...
    auto draw_node = [&](const std::string& name, ImVec2 local_pos, ImVec2 local_size, bool active) {
        ...
        ImVec2 text_pos = ImVec2(
            (p_min.x + p_max.x - text_size.x) * 0.5f,
            (p_min.y + p_max.y - text_size.y) * 0.5f
        );
        draw_list->AddText(text_pos, IM_COL32(240, 240, 250, 255), name.c_str());
    };

    auto draw_arrow = [&](ImVec2 local_start, ImVec2 local_end, const std::string& trigger, bool has_bezier = false, ImVec2 local_cp = ImVec2(0,0)) {
        ...
        if (has_bezier) {
            ...
            for (int i = 0; i <= steps; ++i) {
                ...
                float u = 1.0f - t;
                points[i] = ImVec2(
                    u * u * p_start.x + 2.0f * u * t * p_cp.x + t * t * p_end.x,
        ...
        if (len > 0.0001f) {
            ...
            ImVec2 p_left = ImVec2(p_end.x - dir.x * arrow_size + norm.x * arrow_size * 0.5f, p_end.y - dir.y * arrow_size + norm.y * arrow_size * 0.5f);
            ImVec2 p_right = ImVec2(p_end.x - dir.x * arrow_size - norm.x * arrow_size * 0.5f, p_end.y - dir.y * arrow_size - norm.y * arrow_size * 0.5f);
        ...
        if (!trigger.empty()) {
            ...
            if (has_bezier) {
                ImVec2 p_cp = local_to_screen(local_cp);
                mid = ImVec2(
                    0.25f * p_start.x + 0.5f * p_cp.x + 0.25f * p_end.x,
            ...
            ImVec2 text_rect_min = ImVec2(mid.x - text_size.x * 0.5f - 4.0f * zoom, mid.y - text_size.y * 0.5f - 2.0f * zoom);
            ImVec2 text_rect_max = ImVec2(mid.x + text_size.x * 0.5f + 4.0f * zoom, mid.y + text_size.y * 0.5f + 2.0f * zoom);
            draw_list->AddRectFilled(text_rect_min, text_rect_max, IM_COL32(20, 20, 25, 220), 2.0f * zoom);
            draw_list->AddText(ImVec2(mid.x - text_size.x * 0.5f, mid.y - text_size.y * 0.5f), IM_COL32(180, 180, 190, 255), trigger.c_str());
        }
    };
