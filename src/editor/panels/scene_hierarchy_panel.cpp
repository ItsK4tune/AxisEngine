...
void SceneHierarchyPanel::DrawComponents(entt::registry& reg, entt::entity entity)
{
    if (auto* info = reg.try_get<InfoComponent>(entity)) {
        if (ImGui::CollapsingHeader("Info", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (ImGui::InputText("Name", buffer, sizeof(buffer))) info->name = buffer;
    ...
    if (auto* uiTransform = reg.try_get<UITransformComponent>(entity)) {
        if (ImGui::CollapsingHeader("UI Transform")) {
            ImGui::Checkbox("Use Percentage##UI", &uiTransform->usePercentage);
            ImGui::DragFloat2("Position##UI", &uiTransform->position.x, 1.0f);
            ImGui::DragFloat2("Size##UI", &uiTransform->size.x, 1.0f);
            ...
            ImGui::DragFloat2("Anchor Min##UI", &uiTransform->anchorMin.x, 0.05f);
            ImGui::DragFloat2("Anchor Max##UI", &uiTransform->anchorMax.x, 0.05f);
            ImGui::DragFloat2("Offset Min##UI", &uiTransform->offsetMin.x, 1.0f);
            ImGui::DragFloat2("Offset Max##UI", &uiTransform->offsetMax.x, 1.0f);
        }
    }
