...
void ResourceBrowserPanel::OnImGui(Scene& scene)
{
    ...
    if (ImGui::BeginTabBar("ResourceTabs"))
    {
        ...
        if (ImGui::BeginTabItem("Meshes")) {
            ...
            if (models.empty()) ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "No meshes loaded.");
        ...
        if (ImGui::BeginTabItem("Textures")) {
            ...
            if (textures.empty()) ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "No textures loaded.");
        ...
        if (ImGui::BeginTabItem("Shaders")) {
            ...
            if (shaders.empty()) ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "No shaders loaded.");
        ...
        if (ImGui::BeginTabItem("Audio")) {
            ...
            if (sounds.empty()) ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "No audio loaded.");
            for (const auto& name : sounds) {
                bool selected = (m_SelectedName == name && m_SelectedType == "Audio");
                if (ImGui::Selectable(name.c_str(), selected)) {
                    m_SelectedName = name;
                    m_SelectedType = "Audio";
                    if (m_ActiveSound) { m_ActiveSound->Stop(); m_ActiveSound.reset(); }
                }
            }
            ImGui::EndTabItem();
        }
        ...
        if (ImGui::BeginTabItem("Video")) {
            ...
            if (videos.empty()) ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "No video loaded.");
        ...
        if (ImGui::BeginTabItem("Animations")) {
            ...
            if (anims.empty()) ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "No animations loaded.");
        ...
        if (ImGui::BeginTabItem("Skyboxes")) {
            ...
            if (skyboxes.empty()) ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "No skyboxes loaded.");
        ...
        if (ImGui::BeginTabItem("Fonts")) {
            ...
            if (fonts.empty()) ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "No fonts loaded.");
        ...
        if (ImGui::BeginTabItem("Fragments")) {
            ...
            if (frags.empty()) ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f), "No fragments loaded.");
    ...
    if (m_SelectedName.empty()) {
        ...
        if (m_SelectedType == "Mesh") {
            ...
            if (model) {
                ...
                ImGui::Text("AABB Min: (%.2f, %.2f, %.2f)", model->aabb.minBound.x, model->aabb.minBound.y, model->aabb.minBound.z);
                ImGui::Text("AABB Max: (%.2f, %.2f, %.2f)", model->aabb.maxBound.x, model->aabb.maxBound.y, model->aabb.maxBound.z);

                ImGui::Spacing();
                ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.0f, 1.0f), "3D Wireframe Preview (Drag to Rotate, Scroll to Zoom):");
                ...
                drawList->AddRectFilled(boxStart, ImVec2(boxStart.x + previewSize.x, boxStart.y + previewSize.y), IM_COL32(20, 20, 25, 255), 8.0f);
                drawList->AddRect(boxStart, ImVec2(boxStart.x + previewSize.x, boxStart.y + previewSize.y), IM_COL32(60, 60, 70, 255), 8.0f, 0, 1.5f);
                ...
                for (int g = -5; g <= 5; ++g) {
                    ...
                    drawList->AddLine(
                        ImVec2(boxCenter.x + rp1.x * zoom, boxCenter.y - rp1.y * zoom),
                        ImVec2(boxCenter.x + rp2.x * zoom, boxCenter.y - rp2.y * zoom),
                        IM_COL32(40, 40, 55, 255)
                    );
                    drawList->AddLine(
                        ImVec2(boxCenter.x + rp3.x * zoom, boxCenter.y - rp3.y * zoom),
            ...
            if (texture) {
                ...
                if (texture->id != 0) {
                    ...
                    ImGui::Image((void*)(intptr_t)texture->id, imgSize, ImVec2(0, 0), ImVec2(1, 1), ImVec4(1,1,1,1), ImVec4(0.2f,0.2f,0.2f,1.0f));
                } else {
            ...
            if (shader) {
                ...
                for (const auto& def : rm->GetResourceDefinitions()) {
                    if (def.type == "Shader" && def.name == m_SelectedName) {
                        if (def.properties.count("Vertex")) vsPath = def.properties.at("Vertex");
                        if (def.properties.count("Fragment")) fsPath = def.properties.at("Fragment");
                        break;
                    }
                }
                ...
                if (!fsPath.empty()) {
                    ...
                    ImGui::InputTextMultiline("##ShaderCode", const_cast<char*>(m_CachedShaderCode.c_str()), m_CachedShaderCode.size() + 1, ImVec2(0, 300), ImGuiInputTextFlags_ReadOnly);
                }
            }
        }
            ...
            if (video) {
                ...
                ImGui::Spacing();
                if (ImGui::Button("Play")) video->Play();
                ImGui::SameLine();
                if (ImGui::Button("Pause")) video->Pause();
                ImGui::SameLine();
                if (ImGui::Button("Stop")) video->Stop();

                bool loop = video->IsLooping();
                if (ImGui::Checkbox("Looping", &loop)) video->SetLoop(loop);

                if (video->GetTextureID() != 0) {
                    ImGui::Spacing();
                    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.0f, 1.0f), "Video Feed (Live):");
                    
                    float aspect = (float)video->GetWidth() / (video->GetHeight() > 0 ? (float)video->GetHeight() : 1.0f);
                    ImVec2 vidSize(256.0f, 256.0f / aspect);
                    ImGui::Image((void*)(intptr_t)video->GetTextureID(), vidSize, ImVec2(0, 0), ImVec2(1, 1), ImVec4(1, 1, 1, 1), ImVec4(0.1f, 0.1f, 0.1f, 1.0f));
                }
            } else {
                ...
                for (int i = 0; fontTestText[i] != '\0'; ++i) {
                    ...
                    if (ch.textureID != 0) {
                        ...
                        if (glyphSize.x > 0 && glyphSize.y > 0) {
                            ...
                            ImGui::Image((void*)(intptr_t)ch.textureID, glyphSize, ImVec2(0, 0), ImVec2(1, 1), ImVec4(0.0f, 0.8f, 1.0f, 1.0f), ImVec4(0.1f, 0.1f, 0.15f, 1.0f));
