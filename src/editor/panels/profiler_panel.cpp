...
void ProfilerPanel::OnImGui(Scene& scene)
{
    ...
    ImGui::PlotLines("##FrameTime", m_FrameTimeHistory, HISTORY_SIZE, m_HistoryOffset, overlay, 0.0f,
                     m_MaxFrameTime * 1.5f, ImVec2(-1, 80));

    // FPS graph
    ImGui::Text("FPS");
    char fpsOverlay[32];
    snprintf(fpsOverlay, sizeof(fpsOverlay), "%.0f", m_Fps);
    ImGui::PlotLines("##FPS", m_FpsHistory, HISTORY_SIZE, m_HistoryOffset, fpsOverlay, 0.0f, 0.0f, ImVec2(-1, 60));
    ...
    ImGui::NextColumn();
    ImGui::Text("%.2f ms", m_AvgFrameTime);
    ImGui::NextColumn();

    ImGui::Text("Min Frame Time");
    ImGui::NextColumn();
    ImGui::Text("%.2f ms", m_MinFrameTime < 999.0f ? m_MinFrameTime : 0.0f);
    ImGui::NextColumn();

    ImGui::Text("Max Frame Time");
    ImGui::NextColumn();
    ImGui::Text("%.2f ms", m_MaxFrameTime);
    ImGui::NextColumn();

    ImGui::Text("Avg FPS");
    ImGui::NextColumn();
    ImGui::Text("%.0f", m_AvgFrameTime > 0.01f ? 1000.0f / m_AvgFrameTime : 0.0f);
    ImGui::NextColumn();
