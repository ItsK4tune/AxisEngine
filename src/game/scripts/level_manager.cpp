...
void LevelManager::GenerateGrid(int radius)
{
...
void LevelManager::CreateHexTile(int q, int r)
{
    auto e = m_Scene->createEntity();

    auto &t = m_Scene->registry.emplace<TransformComponent>(e);
    t.position = HexMath::HexToWorld({q, r});
    t.position.y = -0.5f;
...
void LevelManager::SpawnUnit(int q, int r, bool isPlayer)
{
    auto e = m_Scene->createEntity();
...
void LevelManager::HandleClick()
{
    ...
    if (rayCallback.hasHit())
    {
        ...
        if (m_Scene->registry.all_of<ScriptComponent>(hitEntity))
        {
            auto *unitScript = GetScript<Unit>(hitEntity);
            if (unitScript)
            {
                m_SelectedUnit = hitEntity;
                std::cout << "Selected Unit at: " << unitScript->gridPos.q << ", " << unitScript->gridPos.r << std::endl;
                return;
            }
        }
        ...
        {
            // Kiểm tra xem có phải tile không (có transform và mesh nhưng không có script unit)
            // Hoặc có thể thêm TagComponent "Tile" để chắc chắn
