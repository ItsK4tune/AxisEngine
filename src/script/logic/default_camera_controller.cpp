...
void DefaultCameraController::OnUpdate(float dt)
{
    ...
    if (looking && !m_WasLooking) {
        if (hoveringPanel) looking = false;
        else m_WasLooking = true;
    }
    if (panning && !m_WasPanning) {
        if (hoveringPanel) panning = false;
        else m_WasPanning = true;
    }
    if (orbiting && !m_WasOrbiting) {
        if (hoveringPanel) orbiting = false;
        else m_WasOrbiting = true;
    }
    ...
    if (keyboard.GetKey(Key::LeftShift)) speedMultiplier = 2.5f;
    if (keyboard.GetKey(Key::LeftControl)) speedMultiplier = 0.25f;

    float scroll = mouse.GetScrollY();
    if (looking && scroll != 0.0f) {
        m_BaseMoveSpeed += scroll;
        if (m_BaseMoveSpeed < 0.5f) m_BaseMoveSpeed = 0.5f;
        if (m_BaseMoveSpeed > 50.0f) m_BaseMoveSpeed = 50.0f;
    }
    ...
    if (looking) {
        ...
        if (!typing && !isCtrl && !isAlt) {
            ...
            if (keyboard.GetKey(Key::W) || keyboard.GetKey(Key::Up)) targetVelocity += frontNormalized * moveVel;
            if (keyboard.GetKey(Key::S) || keyboard.GetKey(Key::Down)) targetVelocity -= frontNormalized * moveVel;
            if (keyboard.GetKey(Key::A) || keyboard.GetKey(Key::Left)) targetVelocity -= right * moveVel;
            if (keyboard.GetKey(Key::D) || keyboard.GetKey(Key::Right)) targetVelocity += right * moveVel;
            if (keyboard.GetKey(Key::E) || keyboard.GetKey(Key::PageUp)) targetVelocity += worldUp * moveVel;
            if (keyboard.GetKey(Key::Q) || keyboard.GetKey(Key::PageDown)) targetVelocity -= worldUp * moveVel;
        }
    }
    else if (panning) {
    ...
    if (!looking && scroll != 0.0f && !hoveringPanel) {
        ...
        // Update pivot distance relation
...
void DefaultCameraController::FocusOnEntity(entt::entity entity) {
    if (HasComponent<PositionComponent>() && GetScene().registry.valid(entity) && GetScene().registry.all_of<WorldTransformComponent>(entity)) {
