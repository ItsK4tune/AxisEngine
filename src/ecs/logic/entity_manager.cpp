...
entt::entity EntityManager::FindByNameAndTag(Scene &scene,
...
entt::entity
EntityManager::FindByNameTagAndScene(Scene &scene, const std::string &name,
                                     const std::string &tag,
...
EntityManager::FindAllByName(Scene &scene, const std::string &name) {
...
std::vector<entt::entity> EntityManager::FindAllByTag(Scene &scene,
                                                      const std::string &tag) {
  std::vector<entt::entity> results;
  auto view = scene.registry.view<InfoComponent>();
  for (auto entity : view) {
...
std::vector<entt::entity>
EntityManager::FindAllBySceneName(Scene &scene, const std::string &sceneName) {
  std::vector<entt::entity> results;
  auto view = scene.registry.view<InfoComponent>();
  for (auto entity : view) {
...
entt::entity EntityManager::GetCameraByName(Scene &scene,
                                            const std::string &name) {
...
entt::entity EntityManager::GetCameraByTag(Scene &scene,
                                           const std::string &tag) {
  auto view = scene.registry.view<CameraComponent, InfoComponent>();
  for (auto entity : view) {
    if (view.get<InfoComponent>(entity).tag == tag)
...
void EntityManager::SetActiveSkybox(Scene &scene, entt::entity entity) {
  ...
  for (auto skyEntity : view) {
    view.get<SkyboxRenderComponent>(skyEntity).isPrimary =
        (skyEntity == entity);
  }
}

entt::entity EntityManager::CreateEntity(Scene &scene, const std::string &name,
                                         const std::string &tag) {
...
entt::entity EntityManager::CreateEntityWithTransform(Scene &scene,
                                                      const std::string &name,
...
entt::entity EntityManager::CreateEmptyEntity(Scene &scene,
                                              const std::string &name) {
  return CreateEntityWithTransform(scene, name, glm::vec3(0.0f));
}

entt::entity EntityManager::CreateCube(Scene &scene, const std::string &name,
                                       const glm::vec3 &position) {
  return CreateEntityWithTransform(scene, name, position);
}

entt::entity EntityManager::CreateSphere(Scene &scene, const std::string &name,
                                         const glm::vec3 &position) {
  return CreateEntityWithTransform(scene, name, position);
}

entt::entity EntityManager::CreatePlane(Scene &scene, const std::string &name,
                                        const glm::vec3 &position) {
  return CreateEntityWithTransform(scene, name, position);
}

void EntityManager::SetParent(Scene &scene, entt::entity child,
                              entt::entity parent, bool keepWorldTransform) {
  if (!scene.registry.valid(child) || !scene.registry.valid(parent)) {
    LOGGER_ERROR("Scene") << "Invalid child or parent entity";
    return;
  }

  if (!scene.registry.all_of<HierarchyComponent>(child) ||
      !scene.registry.all_of<HierarchyComponent>(parent)) {
  ...
  if (child == parent) {
    LOGGER_ERROR("Scene") << "Attempted to set entity as its own parent: "
                          << (uint32_t)child;
    return;
  }
  ...
  while (current != entt::null) {
    if (current == child) {
      LOGGER_ERROR("Scene")
          << "Cycle detected in hierarchy! Cannot set " << (uint32_t)child
          << " as child of " << (uint32_t)parent;
      return;
    }
  ...
  if (scene.registry.valid(childH.parent) &&
      scene.registry.all_of<HierarchyComponent>(childH.parent)) {
    auto &oldParentH = scene.registry.get<HierarchyComponent>(childH.parent);
    oldParentH.children.erase(std::remove(oldParentH.children.begin(),
                                          oldParentH.children.end(), child),
  ...
  if (auto *w = scene.registry.try_get<WorldTransformComponent>(child))
    w->isDirty = true;
}

void EntityManager::AddChild(Scene &scene, entt::entity parent,
                             entt::entity child, bool keepWorldTransform) {
  SetParent(scene, child, parent, keepWorldTransform);
}

void EntityManager::DestroyEntity(Scene &scene, entt::entity entity,
                                  SceneManager *manager) {
  if (!scene.registry.valid(entity))
    return;

  if (auto *h = scene.registry.try_get<HierarchyComponent>(entity)) {
    if (scene.registry.valid(h->parent) &&
        scene.registry.all_of<HierarchyComponent>(h->parent)) {
    ...
    for (auto child : childrenCopy) {
      if (scene.registry.valid(child) &&
          scene.registry.all_of<HierarchyComponent>(child)) {
        auto &childH = scene.registry.get<HierarchyComponent>(child);
...
void EntityManager::DestroyEntityWithChildren(Scene &scene, entt::entity entity,
                                              SceneManager *manager) {
  if (!scene.registry.valid(entity))
    return;
