...
namespace
{
    ...
    unsigned int TextureFromFile(const char *path, const std::string &directory, const aiScene *scene, bool deferred, Texture *outTex = nullptr)
    {
        ...
        if (data)
        {
            ...
            tm.TexImage2D(TextureType::Texture2D, 0, InternalFormat::RGBA8, width, height, 0, TextureFormat::RGBA, DataType::UnsignedByte, data);
    ...
    std::vector<Texture> loadMaterialTextures(std::vector<Texture> &textures_loaded,
                                              ...
                                              const aiScene *scene, bool deferred)
    {
                                      ...
                                      int &boneCount)
    {
        ...
        if (mesh->mNumBones > 0)
        {
            LOGGER_INFO("Model") << "Mesh " << mesh->mName.C_Str() << " has " << mesh->mNumBones << " bones (Total unique bones in model: " << boneCount << ")";
        }
    }
                     ...
                     std::unordered_map<std::string, BoneInfo> &boneInfoMap, int &boneCount, bool deferred)
    {
        ...
        if (mesh->mMaterialIndex >= 0)
        {
            ...
            std::vector<Texture> diffuseMaps = loadMaterialTextures(textures_loaded, material, aiTextureType_BASE_COLOR, "texture_diffuse", directory, scene, deferred);
            if (diffuseMaps.empty())
            {
                diffuseMaps = loadMaterialTextures(textures_loaded, material, aiTextureType_DIFFUSE, "texture_diffuse", directory, scene, deferred);
                if (diffuseMaps.empty())
                {
                    aiColor4D color;
                        ...
                        aiGetMaterialColor(material, AI_MATKEY_COLOR_DIFFUSE, &color) == AI_SUCCESS)
                    {
                        ...
                        if (!deferred)
                        {
                            ...
                            tm.TexImage2D(TextureType::Texture2D, 0, InternalFormat::RGBA8, 1, 1, 0, TextureFormat::RGBA, DataType::UnsignedByte, data);
                            tm.TexParameteri(TextureType::Texture2D, TextureParameter::WrapS, static_cast<int>(TextureWrap::Repeat));
                            tm.TexParameteri(TextureType::Texture2D, TextureParameter::WrapT, static_cast<int>(TextureWrap::Repeat));
            ...
            std::vector<Texture> specularMaps = loadMaterialTextures(textures_loaded, material, aiTextureType_SPECULAR, "texture_specular", directory, scene, deferred);
            ...
            std::vector<Texture> normalMaps = loadMaterialTextures(textures_loaded, material, aiTextureType_NORMALS, "texture_normal", directory, scene, deferred);
            if (normalMaps.empty())
                normalMaps = loadMaterialTextures(textures_loaded, material, aiTextureType_HEIGHT, "texture_normal", directory, scene, deferred);
            textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());

            std::vector<Texture> metallicMaps = loadMaterialTextures(textures_loaded, material, aiTextureType_METALNESS, "texture_metallic", directory, scene, deferred);
            textures.insert(textures.end(), metallicMaps.begin(), metallicMaps.end());

            std::vector<Texture> roughnessMaps = loadMaterialTextures(textures_loaded, material, aiTextureType_DIFFUSE_ROUGHNESS, "texture_roughness", directory, scene, deferred);
            textures.insert(textures.end(), roughnessMaps.begin(), roughnessMaps.end());

            std::vector<Texture> aoMaps = loadMaterialTextures(textures_loaded, material, aiTextureType_AMBIENT_OCCLUSION, "texture_ao", directory, scene, deferred);
            if (aoMaps.empty())
                aoMaps = loadMaterialTextures(textures_loaded, material, aiTextureType_LIGHTMAP, "texture_ao", directory, scene, deferred);
...
void Model::UploadToGPU()
{
    ...
    for (auto &tex : textures_loaded)
    {
        if (tex.id == 0 && tex.pixelData)
        {
            ...
            tm.TexImage2D(TextureType::Texture2D, 0, InternalFormat::RGBA8, tex.width, tex.height, 0, TextureFormat::RGBA, DataType::UnsignedByte, tex.pixelData);
            tm.GenerateMipmap(TextureType::Texture2D);
            tm.TexParameteri(TextureType::Texture2D, TextureParameter::WrapS, static_cast<int>(TextureWrap::Repeat));
            tm.TexParameteri(TextureType::Texture2D, TextureParameter::WrapT, static_cast<int>(TextureWrap::Repeat));
            tm.TexParameteri(TextureType::Texture2D, TextureParameter::MinFilter, static_cast<int>(TextureFilter::LinearMipmapLinear));
