...
bool MaterialRenderer::SetupMaterialUniforms(Shader *shader, AxisMaterialComponent* material, const RenderSceneData& sceneData, const glm::vec4& tintColor, bool debugNoTexture, bool isWireframe) {
    ...
    if (locs.u_isWireframe != -1) shader->setBool(locs.u_isWireframe, isWireframe);
    ...
    if (material) {
        ...
        if (locs.u_UVOffset != -1) shader->setVec2(locs.u_UVOffset, mat.desc.uvOffset);
        ...
        if (locs.u_RoughnessMap != -1) shader->setInt(locs.u_RoughnessMap, 3);
