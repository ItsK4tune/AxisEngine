...
void ShadowRenderer::PerformShadowPass(const RenderSceneData& sceneData)
{
    ...
    for (const auto& light : sceneData.lights)
    {
        ...
        if (light.type == RenderLightType::Directional)
            shadowCastingDirLights.push_back(&light);
    ...
    if (shadowCastingDirLights.empty() && shadowCastingPointLights.empty() && shadowCastingSpotLights.empty())
        return;
    ...
    int numDirShadows = (m_ShadowMode == 2)
                            ? (std::min)((int)shadowCastingDirLights.size(), Shadow::MAX_DIR_LIGHTS_SHADOW)
                            : (shadowCastingDirLights.empty() ? 0 : 1);
    ...
    for (int lightIdx = 0; lightIdx < numDirShadows; ++lightIdx)
    {
        ...
        m_MainQueue.Submit([this, lightIdx]() {
                ...
                [this, &shadowQueue, startIdx, endIdx, &threadQueue = m_ThreadQueues[i], shaderDir, lightIdx,
                 lightFrustum, camPos, lightingMode]() {
                    ...
                    for (size_t k = startIdx; k < endIdx; ++k)
                    {
                        ...
                        if (lightingMode == LightingMode::Bake && !item.isStatic)
                            continue;
                        ...
                        if (activeModel)
                        {
                            threadQueue.Submit([shaderDir, worldMat, activeModel, hasAnim = item.hasAnimation,
                                                bones = item.boneMatrices]() {
                                shaderDir->setMat4("u_Model", worldMat);
                                shaderDir->setBool("u_HasAnimation", hasAnim);
                                if (hasAnim && bones)
                                    shaderDir->setMat4Array("u_FinalBonesMatrices", *bones);
                        ...
                        for (size_t k = startIdx; k < endIdx; ++k)
                        {
                            ...
                            if (activeModel)
                            {
                                threadQueue.Submit([shaderPoint, worldMat, activeModel, hasAnim = item.hasAnimation,
                                    ...
                                    if (hasAnim && bones)
                                        shaderPoint->setMat4Array("u_FinalBonesMatrices", *bones);
    ...
    if (shaderSpot)
    {
        ...
        for (int sIdx = 0; sIdx < numSpotShadows; ++sIdx)
        {
            ...
            // Bake mode optimization
            bool needsUpdate = (lightingMode != LightingMode::Bake) ||
                               (light->version != m_LastLightVersionsSpot[sIdx]) || (!m_ShadowMapInitializedSpot[sIdx]);
                        ...
                        for (size_t k = startIdx; k < endIdx; ++k)
                        {
                            ...
                            if (activeModel)
                            {
                                threadQueue.Submit([shaderSpot, worldMat, activeModel, hasAnim = item.hasAnimation,
                                    ...
                                    if (hasAnim && bones)
                                        shaderSpot->setMat4Array("u_FinalBonesMatrices", *bones);
