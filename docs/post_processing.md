...
> **Note**: Post Process shaders typically use a simple screen-quad vertex shader (often `ppt.vs` or similar) that passes valid texture coordinates.
...
void GameState::Init()
{
    ...
    if (invertShader)
    {
        m_App->GetPostProcess().AddEffect(invertShader, 0, 0, 1.0f, 1.0f); // 1.0f = full screen relative
    }
}
```

## 3. Pipeline Order
Effects are applied in the order they are added. The output of one effect becomes the input texture of the next.

## 4. API Reference

### `PostProcessPipeline::AddEffect(Shader* shader, float x, float y, float w, float h)`
